  ...
  useEffect(() => {
    ...
    const loop = () => {
      setDisplay(prev => {
        ...
        if (Math.abs(dist) < 0.5) return target; // Snap when close
        return prev + (dist * 0.15); // Smooth ease-out
      });
...
const ROICalculator: React.FC<ROICalculatorProps> = ({ onOpenDemo }) => {
  const [volume, setVolume] = useState(5000);
  const [credit, setCredit] = useState(15000);
  const [crop, setCrop] = useState<'Tea' | 'Dairy' | 'Avocado'>('Tea');
  ...
  // Estimation Logic
  const baseLoss = (volume * 1000) + (credit * 200);
  const rawEstimatedLoss = Math.floor(baseLoss * cropFactors[crop]);
  const rawRecoverable = Math.floor(rawEstimatedLoss * 0.7);


  // Log interaction to backend and GTM (debounced)
  useEffect(() => {
    const timer = setTimeout(() => {
      // 1. Log to Backend for internal analytics
      ...
      // 2. Push to GTM dataLayer for marketing/behavioral tracking
  ...
  return (
    <section id="roi" className="bg-surface-light dark:bg-surface-dark py-20 border-y border-gray-200 dark:border-white/5 relative">
      {/* Custom styles for the range slider to enable thumb animation */}
      <style>{`
        ...
        .slider-thumb::-webkit-slider-thumb:hover {
          ...
          box-shadow: 0 4px 12px rgba(97, 184, 105, 0.4);
        }
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10">
          <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Profit Recovery & ROI Estimator</h2>
          <p className="text-gray-600 dark:text-gray-400">Calculate how much value you can unlock by automating compliance and payments.</p>
        </div>
        ...
        <div className="bg-white dark:bg-[#151d16] rounded-lg shadow-xl border border-gray-200 dark:border-white/10 p-6 transform transition-all hover:shadow-2xl duration-500">
          ...
          <div className="grid gap-12 md:grid-cols-2 mb-8">
            ...
            <div className="space-y-10">
              ...
              <div className="group">
                <div className="flex justify-between mb-4">
                  <label htmlFor="roi-credit" className="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-primary transition-colors">Avg Credit per Farmer</label>
            ...
            <div className="flex flex-col justify-between p-6 rounded-lg bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/5 relative overflow-hidden group/card">
              ...
              <div className="text-center relative z-10 space-y-1">
                <p className="text-xs text-gray-600 uppercase tracking-widest font-semibold">Cost of Continued Inaction</p>
                ...
                <p className="text-xs text-red-600 italic">Unverified Inputs & Compliance Gaps</p>
              </div>

              <div className="my-6 flex items-center justify-center">
                <div className="h-px bg-gray-200 dark:bg-white/10 w-full"></div>
                <div className="mx-3 text-gray-400 text-[10px] font-bold tracking-widest">ROI</div>
                <div className="h-px bg-gray-200 dark:bg-white/10 w-full"></div>
              </div>

              <div className="text-center relative z-10 space-y-1">
                <p className="text-xs text-gray-600 uppercase tracking-widest font-semibold">Recoverable Profit with Vunachain</p>
                ...
                <p className="text-xs text-green-700 dark:text-green-500 italic">Automated Compliance & Payouts</p>
              </div>
